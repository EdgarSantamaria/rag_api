...
async def perform_rag(youtube_url, query, pinecone_api_key, openai_client):
    try:
        ...
        if len(vector_exists['matches']) > 0:
            print('Vector exists.')
            return query_embedding(query, pinecone_index, video_id, namespace, openai_client, index_name)
        else:
            print('Vector does not exist. Creating a new vector store.')
            store_video_data(youtube_url, index_name, namespace, video_id)
            return query_embedding(query, pinecone_index, video_id, namespace, openai_client, index_name)
...
def store_video_data(youtube_url, index_name, namespace, video_id):
...
def query_embedding(query, pinecone_index, video_id, namespace, openai_client , index_name):
    # Initialize Pinecone
    pc = Pinecone(api_key=pinecone_api_key)
    # Connect to Pinecone index
    pinecone_index = pc.Index(index_name)
